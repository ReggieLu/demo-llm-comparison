...
client_llm_jp = openai.OpenAI(
        api_key=os.environ["SAMBANOVA_API_KEY_LLM_JP"],
...
def llm_jp_172b():
    ...
    def generate():
        try:
            completion = client_llm_jp.chat.completions.create(
                model="LLM_JP_172B",  # Verify this is the correct model name
                messages=[
            ...
            logger.error(f"LLM-JP-172B Error: {str(e)}")
            error_data = json.dumps({"error": f"Error calling LLM-JP-172B API: {str(e)}"})
                ...
                try:
                    ...
                    for chunk in completion:
                        if chunk.choices[0].delta.content:
                            ...
                            response_queues["llm_jp"].put((content, duration))
                except Exception as e:
